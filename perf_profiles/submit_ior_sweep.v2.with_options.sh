...
export RESULTS_ROOT="${RESULTS_ROOT:-/global/cfs/cdirs/m5314/zhen.peng/Data/ior_results_root}"

echo "RESULTS_ROOT: ${RESULTS_ROOT}"

# Optional: module load IOR if needed (leave empty if `ior` is already in PATH).
export MODULE_LOAD_CMD="${MODULE_LOAD_CMD:-}"

# Optional: set a specific ior binary.
export IOR_BIN="${IOR_BIN:-/global/homes/j/johnpzh/local/bin/ior}"
...
export PATTERNS_CSV="${PATTERNS_CSV:-sequential,random}"
...
ARRAY_CHUNK_SIZE="${ARRAY_CHUNK_SIZE:-0}"
...
############################################
# Parse command line arguments for --storage, --nodes, and --storageRoot
############################################
...
if [[ -z "${STORAGE_OPTION}" ]]; then
  echo "Error: --storage argument must be provided"
  exit 1
fi
if [[ -z "${NODES_OPTION}" ]]; then
  echo "Error: --nodes argument must be provided"
  exit 1
fi
if [[ -z "${STORAGEROOT_OPTION}" ]]; then
  echo "Error: --storageRoot argument must be provided"
  exit 1
fi

case "${STORAGE_OPTION}" in
  beegfs|ssd|nfs|tmpfs) ;;
  *)
    echo "Error: --storage must be one of: beegfs, ssd, nfs, tmpfs (got '${STORAGE_OPTION}')" >&2
    exit 1
    ;;
esac

set -x
submit_one "${STORAGE_OPTION}" "${NODES_OPTION}" "${STORAGEROOT_OPTION}"
set +x
